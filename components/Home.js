...
function Home() {
  ...
  useEffect(() => {
    fetch('https://backendmovieapp-one.vercel.app/movies').then(res => res.json()).then(data => {
  ...
  const movies = moviesData.map((data, i) => {
    ...
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={`${data.overview.slice(0, 80)}...
    `} poster={data.poster_path} voteAverage={data.vote_average} voteCount={data.vote_count} />;
  });
